...
public CCanvas(Container _Owner)
{
	...
	Owner=_Owner; setBackground(Owner.getBackground());
	Tip=new CTip(this);
...
{
	IsAlive=false;
	try { T.resume(); }catch(Exception e){}
}
...
private void layoutTip()
{
 ...
 Container parent=null;
	...
	try
	{
		...
		{
			px+=parent.getLocation().x; py+=parent.getLocation().y;
			parent=parent.getParent();
		}

		g=getGraphics(); if (g==null) g=parent.getGraphics();
		if (g==null) return;

		Tip.setSize(1, 1); Tip.setLocation(-999, -999);
		parent.add(Tip, 0);
		
		g.setFont(new Font("Dialog", 0, 12));
		fm=g.getFontMetrics();
		w=fm.stringWidth(Tip.Text);
		w=8+w; h=16; Tip.setSize(w, h);
		
		if (ME!=null) { mx=ME.x; my=ME.y; }
		x=px+getLocation().x+mx+d; y=py+getLocation().y+my+d;
 ...
 IGCLClickListener l=null;
	...
	try
	{
		if (!evt.metaDown())
		{
			...
			{
				l=(IGCLClickListener)Owner; l.gcClicked(evt);
			}
		}
	}
	catch(Exception e1){}
