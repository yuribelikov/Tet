...
public void addWhere(int _what, int _fixpos)
{
	...
	switch(_what)
	{
		case YEAR:{ SQL+=Integer.toString(pos_year[0]+1)+","+Integer.toString(pos_year[1]-pos_year[0]+1)+")";length=(pos_year[1]-pos_year[0]);break;}	
		case MONTH:{ SQL+=Integer.toString(pos_month[0]+1)+","+Integer.toString(pos_month[1]-pos_month[0]+1)+")";length=(pos_month[1]-pos_month[0]);break;}	
		case DAY:{ SQL+=Integer.toString(pos_day[0]+1)+","+Integer.toString(pos_day[1]-pos_day[0]+1)+")";length=(pos_day[1]-pos_day[0]);break;}	
		case HOUR:{ SQL+=Integer.toString(pos_hour[0]+1)+","+Integer.toString(pos_hour[1]-pos_hour[0]+1)+")";length=(pos_hour[1]-pos_hour[0]);break;}	
		case MINUTE:{ SQL+=Integer.toString(pos_minute[0]+1)+","+Integer.toString(pos_minute[1]-pos_minute[0]+1)+")";length=(pos_minute[1]-pos_minute[0]);break;}	
		case SECOND:{ SQL+=Integer.toString(pos_second[0]+1)+","+Integer.toString(pos_second[1]-pos_second[0]+1)+")";length=(pos_second[1]-pos_second[0]);break;}	
	}
	length++;
...
public  Object getDateTime(int return_type,String _date)
{
 int year=0, month=0, day=0, hour=0, minute=0, second=0;
	...
	switch (return_type)
	{
		...
		case DATE: {return calendar.getTime();}
	}
...
public String getStandartString(int _type, Object obj)
{
	...
	{
		case STRING: { break;}
		case DATE: {calendar.setTime((Date)obj); break;}
		case CALENDAR: { calendar=(Calendar) obj; break;}
	}
	calendar.add(Calendar.MILLISECOND,-calendar.get(calendar.ZONE_OFFSET)-calendar.get(calendar.DST_OFFSET));
