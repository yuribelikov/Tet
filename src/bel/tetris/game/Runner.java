...
// - - - - - - - - key events - - - - - - - - -
		if (_Evt.getName().equals("KeyPressed"))
		{
			...
			{
				Cups[0].processAction("Pause"); Cups[1].processAction("Pause");
			}
		}
...
// - - - - - - - - menu and window actions - - - - - - - 
		...
		{
			Cups[0].processAction("Quit"); Cups[1].processAction("Quit");
			if (YesNoBox.showBox("Подтвердите", "Вы уверены, что хотите выйти из программы ?"))
			{
				Cups[0].finishGame(); Cups[1].finishGame();
				destroy();
			}
			else
			{
				Cups[0].processAction("No"); Cups[1].processAction("No");
			}
		}
	}
	catch(Exception e)
...
//Log.LogEnabled=true;
		...
		setTitle("ТЕТРИС"); setLayout(null); setResizable(false);
		...
		GameMenu=new Menu("Игра"); MB.add(GameMenu); GameMenu.setFont(f);
		
		NewGameMI=new MenuItem("Новая Игра"); NewGameMI.setName("NewGame");
		GameMenu.add(NewGameMI); NewGameMI.setFont(f);
		NewGameMI.addActionListener((ActionListener)EventListener);
		GameMenu.addSeparator();
		
		ScoresMI=new MenuItem("Зал Славы"); ScoresMI.setName("Scores");
		GameMenu.add(ScoresMI); ScoresMI.setFont(f);
		ScoresMI.addActionListener((ActionListener)EventListener);
		GameMenu.addSeparator();
		
		OptionsMI=new MenuItem("Настройки"); OptionsMI.setName("Options");
		GameMenu.add(OptionsMI); OptionsMI.setFont(f);
		OptionsMI.addActionListener((ActionListener)EventListener);
		GameMenu.addSeparator();
		
		QuitMI=new MenuItem("Выход"); QuitMI.setName("Quit");
		GameMenu.add(QuitMI); QuitMI.setFont(f);
		...
		Options=new COptions(); loadOptions();
		Cups=new CCup[2]; Cups[0]=new CCup(this, 1); Cups[1]=new CCup(this, 2);
	}
	catch(Exception e)
	{
