...
private void mousePressed(int _X, int _Y, boolean _RightButton)
{
	try
	{
		...
		int x=scrSize.width/2-w/2; int y=scrSize.height/2-h/2+10;
...
public void processAction(String _Action, CEvent _Evt)
{
	try
	{
		...
		if (_Action.startsWith("SaveLevel"))
		{
			Task="SaveLevel";
		}
		else
		if (_Action.startsWith("ClearLevel"))
		{
			Contents=new int[H][W]; paint();
		}
		else
		if (_Action.startsWith("MousePressed"))
		{
...
Log.log("Task="+Task);
			...
			if (Task.equals("SaveLevel")) saveLevel();
